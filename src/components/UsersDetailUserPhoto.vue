<template>
  <div class="position-relative max-w-200 white-space-nowrap">
    <div
      class="w-200 h-200 border-radius-round overflow-hidden border-solid border-4 border-gray-border"
    >
      <img
        v-if="photoSrc !== ''"
        class="object-fit-cover h-200 w-200"
        :src="photoSrc"
      />
      <template v-else>
        <div
          class="d-flex align-center justify-center h-192 w-192 background-gray-header"
        >
    ...
    <div v-show="mode !== 'view'" class="position-absolute right-0 bottom-2">
      <button
        class="w-52 h-52 background-primary outline-none d-flex align-center justify-center border-radius-round cursor-pointer"
